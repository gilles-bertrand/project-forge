...
const E2E_DIR = path.resolve(ROOT_DIR, "@apps/e2e");

function log(message: string) {
  console.log(`\x1b[36m[e2e-setup]\x1b[0m ${message}`);
}

function error(message: string) {
  console.error(`\x1b[31m[e2e-setup]\x1b[0m ${message}`);
}

function success(message: string) {
  console.log(`\x1b[32m[e2e-setup]\x1b[0m ${message}`);
}

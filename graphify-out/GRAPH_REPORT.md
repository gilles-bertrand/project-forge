...
- `Boilerplate Logo (Hexagonal Gradient)` --legacy_brand_predecessor_of--> `SprintForge App Shell (Sidebar + Topbar + Content)`  [AMBIGUOUS]
  @apps/front/public/assets/img/boilerplate_logo.png → docs/figma-screenshots/01-dashboard.png
- `Dashboard Screen (Tableau de bord)` --semantically_similar_to--> `Dashboard Light Mode (Tableau de bord)`  [INFERRED] [semantically similar]
  docs/figma-screenshots/01-dashboard.png → /Users/gilles/www/projects/project-forge/docs/figma-screenshots/24-dashboard-light-mode.png
- `Kanban Screen` --semantically_similar_to--> `Kanban Light Mode (5 columns: À faire, En cours, À tester, UAT, Terminé)`  [INFERRED] [semantically similar]
  docs/figma-screenshots/04-kanban.png → /Users/gilles/www/projects/project-forge/docs/figma-screenshots/25-kanban-light-mode.png
- `SprintForge App Shell (Sidebar + Topbar + Content)` --uses_layout--> `Users Screen (Utilisateurs - team management)`  [EXTRACTED]
  docs/figma-screenshots/01-dashboard.png → /Users/gilles/www/projects/project-forge/docs/figma-screenshots/08-users.png
- `SprintForge App Shell (Sidebar + Topbar + Content)` --precedes--> `Login Screen (Connexion)`  [INFERRED]
  docs/figma-screenshots/01-dashboard.png → /Users/gilles/www/projects/project-forge/docs/figma-screenshots/10-login.png

## Hyperedges (group relationships)
- **SprintForge Scrum Workflow (Backlog -> USM -> Sprints -> Kanban -> Dashboard)** —  [INFERRED 0.90]
- **Shared App Chrome (Sidebar + Topbar across all screens)** —  [EXTRACTED 1.00]
- **Shared Task Card Pattern (ID + Title + Labels + Story + Assignee + Points)** —  [INFERRED 0.90]
- **Core SprintForge workflow screens** —  [INFERRED 0.85]
- **Account and access management surfaces** —  [INFERRED 0.85]
- **Light mode design variants** —  [EXTRACTED 1.00]
- **Add-entity modal family (all 'Add X' modals share layout, footer pattern, dark theme)** —  [INFERRED 0.90]
- **Required fields with red asterisk indicator across add-entity modals** —  [EXTRACTED 0.90]
- **Entities scoped to a Projet selection (Task, UserStory, Epic all require Projet)** —  [EXTRACTED 0.90]
- **Detail modals family (shared chrome pattern)** —  [INFERRED 0.90]
- **Sprint create/edit form pair** —  [INFERRED 0.92]
- **Project context navigation flow** —  [INFERRED 0.85]
- **SprintForge Migration Roadmap** —  [INFERRED 0.97]
- **Session Handoffs Chain** —  [INFERRED 0.97]
- **ADR Decisions Bundle** —  [INFERRED 0.95]
- **SprintForge backend architecture** —  [INFERRED 0.96]
- **SprintForge frontend addons architecture** —  [INFERRED 0.96]
- **JSON:API + Zod + Route<T> convention** —  [INFERRED 0.97]
...
- `Boilerplate Logo (Hexagonal Gradient)` → `SprintForge App Shell (Sidebar + Topbar + Content)`  [AMBIGUOUS]
  @apps/front/public/assets/img/boilerplate_logo.png · relation: legacy_brand_predecessor_of
- `Project Selector Dropdown` → `Project Detail Modal`  [AMBIGUOUS]
   · relation: navigates_to
- `Task Meta right panel` → `Sprint History Modal`  [AMBIGUOUS]
...
- **What is the exact relationship between `Boilerplate Logo (Hexagonal Gradient)` and `SprintForge App Shell (Sidebar + Topbar + Content)`?**
  _Edge tagged AMBIGUOUS (relation: legacy_brand_predecessor_of) - confidence is low._
- **What is the exact relationship between `Project Selector Dropdown` and `Project Detail Modal`?**

...
function parseFilters(query: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [k, v] of Object.entries(query)) {
    ...
    if (!field) continue;
    if (field === "date.gte") {
      const existing = (where.date as Record<string, unknown> | undefined) ?? {};
      where.date = { ...existing, $gte: new Date(v as string) };
    } else if (field === "date.lte") {
      const existing = (where.date as Record<string, unknown> | undefined) ?? {};
      where.date = { ...existing, $lte: new Date(v as string) };
    } else {
      where[field] = v;
    }
  }
...
export function parseListQuery(
  ...
  return { where, orderBy, page, limit, offset };
}
